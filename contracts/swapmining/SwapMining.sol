...
contract SwapMining is SafeOwnable {
    ...
    struct PoolInfo {
        ...
        uint256 allocGIBXAmount; // How many GIBXs
    ...
    function getGIBXReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getGIBXReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        ...
        pool.allocGIBXAmount = pool.allocGIBXAmount.add(gibxReward);
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        address pair = GIBXLibrary.pairFor(address(factory), input, output);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        } else if (IGIBXFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocGIBXAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocGIBXAmount = pool.allocGIBXAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
    ...
    function getUserReward(uint256 _pid, address _user) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getGIBXReward(pool.lastRewardBlock);
            uint256 gibxReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocGIBXAmount.add(gibxReward)).mul(user.quantity).div(pool.quantity));
        }
        //GIBX available to users, User transaction amount
    ...
    function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256){
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IGIBXPair(pool.pair).token0();
        address token1 = IGIBXPair(pool.pair).token1();
        uint256 gibxAmount = pool.allocGIBXAmount;
        uint256 blockReward = getGIBXReward(pool.lastRewardBlock);
        uint256 gibxReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
